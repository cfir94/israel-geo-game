...
/* ---------- אתגר הכיתה ----------
   יעד שבועי משותף. גדל עם גודל הכיתה כדי שיישאר מאתגר, ומעוגל
   למספר עגול כדי שיהיה קל לזכור אותו. */
function classGoal(members) {
  const n = Math.max(1, members || 1);
  return Math.max(200, Math.round(n * 60 / 50) * 50);
}
